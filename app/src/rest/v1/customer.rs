...
pub async fn save(input: web::Json<Customer>) -> Result<HttpResponse> {
    ...
    Ok(HttpResponse::Ok().json(Customer{
        ...
        tags: vec!(CustomerTag{
            id: String::from("1"),
            name: String::from("CachorroNoLocal"),
            tag_type: String::from("Principal"),
        })
    }))
}
...
pub async fn query(query: web::Query<QueryCustomer>) -> impl Responder {
    HttpResponse::Ok().body(format!("Query all! : document[{}] name[{}], secondName[{}], personType[{}], tagIds[{:?}], {}, {}",
        query.document,
        query.name,
        query.second_name,
        query.person_type,
...
pub async fn query_tags(query: web::Query<QueryTags>) -> impl Responder {
    HttpResponse::Ok().body(format!("Query tags! : name[{}], type[{}]", query.name, query.query_type))
}
